...
class Game extends Component {
    ...
    result_false = () => {
        ...
        const newGame = {
            'names': this.props.userName,
            'status': status,
            'accountU': String(this.scoresUser),
            'accountC': String(this.scoresComputer)
        }
        axios.post('http://localhost:3001/api/game', newGame)
            .catch((err) => {
                console.log(err)
            })
        ...
        return (
            <>
                <table className="score">
                    <tr>
                        <td> <span> {userName} : {this.scoresUser} </span> </td>
                        <td> VS </td>
                        <td> <span> Комп'ютер : {this.scoresComputer} </span> </td>
                    </tr>
                </table>
                <div className="game">
                    <table>
